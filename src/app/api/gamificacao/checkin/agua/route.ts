...
export async function POST(req: NextRequest) {
  ...
  try {
    const prev = await query(
      `SELECT streak_atual FROM streaks WHERE user_id=$1 AND data=$2 AND tipo='agua' LIMIT 1`,
    ...
    if (newStreak >= 30) {
      await query(
        ...
        [userId, 'Água 30 Dias', '💧', 30]
      );
      unlockedBadge = { nome: 'Água 30 Dias', emoji: '💧' };
    }
    // Global streak
